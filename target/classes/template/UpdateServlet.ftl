...
@WebServlet("/${className?cap_first}UpdateServlet")
public class ${className?cap_first}UpdateServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        ${className?cap_first} ${className?uncap_first} = new ${className?cap_first}(id<#list columns as column><#if column?index == 0><#continue></#if><#if column?index gt 0>,</#if><#if column.columnClassName == "Date">null<#else>${column.columnName?replace("_([a-z])"," $1","r")?capitalize?replace(" ","","r")?uncap_first}</#if></#list>);

        if ( ${className?uncap_first}Service.update${className?cap_first}Service(${className?uncap_first}) ) {
