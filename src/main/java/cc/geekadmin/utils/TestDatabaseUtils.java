...
public class TestDatabaseUtils {
    ...
    public Database getDatabase(String driver, String url, String username, String password) {
        // 创建数据库连接
        Connection connection = JDBC.getConnection(driver, url, username, password);
        if (connection == null) {
            return null;
        }
        // 创建数据库类型
        Database database = new Database(getDatabaseName(connection));
        // 创建表集合
        List<Table> tables = new ArrayList<Table>();
        // 获取表详细信息
        for (String table : getTableName(connection)) {
            tables.add(new Table(table, getTableRemarksName(connection, table), getColumns(connection, database.getDataBasesName(), table)));
        }
        // 插入表详细信息
     ...
     * @return String[]
     */
    public ArrayList<String> getTableName(Connection connection) {
     ...
     * @return String 注释信息
     */
    public String getTableRemarksName(Connection connection, String tableName) {
        String tableRemarksName = "";
        try {
            PreparedStatement preparedStatement = connection.prepareStatement("SELECT table_comment FROM information_schema.TABLES WHERE table_schema = ( SELECT DATABASE () ) AND table_name = '" + tableName + "'");
            ResultSet resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
     ...
     */
    public ArrayList<Column> getColumns(Connection connection, String databaseName, String tableName) {
        ...
        try {
            ...
            PreparedStatement preparedStatement = connection.prepareStatement("select * from " + tableName + " where 1 = 2");
